...
def searchResults(request):
    if 'query' in request.GET:
        if request.GET['query'] == '':
            return redirect('stWeb.views.searchAdv')
...
def single(request, id):
    context = {
        'obj': Starodruk.objects.get(id=id),
        'title': 'Starodruki',
        'request': request,
        'templates': templates,
        'highlight': [],
...
def showThumbnails(request, id, pageNo):
    parent = Starodruk.objects.get(id=id)
    ...
    context = {
        ...
        'request': request,
        'templates': templates,
        'chnagePageLink': 'stWeb.views.showThumbnails',
        'pt': pageTranslate,
        }
...
def zoom(request, id, pageNo):
    ...
    context = {
        ...
        'templates': templates,
        'sharebox': 1,
        'chnagePageLink': 'stWeb.views.zoom',
        'pt': pageTranslate,
        }
...
def double(request, id, pageNo):
    ...
    context = {
        ...
        'templates': templates,
        'sharebox': 1,
        'chnagePageLink': 'stWeb.views.double',
        'pt': pageTranslate,
        }
    return render_to_response(templates['double'], context)
    
def listNames(request, letter, pageNo):
    if letter != '#':
        qs = Starodruk.objects.filter(autor_dok__istartswith=letter).values('autor_dok').distinct().order_by('autor_dok')
    qs = sorted(qs)
    p = None
    currPage = None
    actQuery = []
    ...
    context = {
        'alphabet': alphabet,
        'actLet': letter,
        'activeLetters': Starodruk.objects.all().extra(select={'letter': 'UPPER(LEFT(autor_dok,1))'}).values_list('letter', flat=True).distinct(),
        'qs': actQuery,
        'page': currPage,
        'pages': pages,
        'p': p,
...
#         return HttpResponseNotFound()
    ...
    try:
        if type == 'full':
            obj = Starodruk.objects.get(id=objId)        
            ...
            obj = Starodruk.objects.get(id=objId)
            system_path = '/srv/www/python/django-1.5-env/www/starodruki/stWeb/static/'
            file_path = system_path + files[0]
            if not os.path.exists(file_path):
                file_path = system_path + (Starodruk.objects.get(id=files[0])).title_page_path_m
            fsock = open(file_path,"r")
            file_name = os.path.basename(file_path)
            file_size = os.path.getsize(file_path)
            print "file size is: " + str(file_size)
